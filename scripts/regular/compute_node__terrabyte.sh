...
then
    python ../../benchmarking_terramind_on_biomassters.py \
	   ...
	   -bs $batch_size \
	   -pthd $dataset \
	   -pthl  $log_dir > $backbone_size"__frozen__sensor_"$sensor_i$(date +"__%Y_%m_%d__%H_%M_%S")"_.out"
else
    python ../../benchmarking_terramind_on_biomassters.py \
	   -sr $sensor_i \
	   -bcsz  $backbone_size \
	   ...
	   -bs $batch_size \
	   -pthd $dataset \
	   -pthl  $log_dir > $backbone_size"__not_frozen__sensor_"$sensor_i$(date +"__%Y_%m_%d__%H_%M_%S")"_.out" 
fi
